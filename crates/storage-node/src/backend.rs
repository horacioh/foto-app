...
pub trait Backend: Send + Sync + 'static {
    ...
    /// Whole blob, chunks concatenated in order. Range support lands with video (phase 4).
    fn get(&self, id: BlobId) -> BoxFuture<'_, Result<Bytes, BackendError>>;
...
impl Backend for LocalDisk {
    fn put_chunk(
        ...
        Box::pin(async move {
            ...
            tokio::fs::write(dir.join("total"), total.to_string()).await?;
    ...
    fn get(&self, id: BlobId) -> BoxFuture<'_, Result<Bytes, BackendError>> {
        Box::pin(async move {
            ...
            let mut out = Vec::new();
            for n in 0..total {
                out.extend(tokio::fs::read(dir.join(format!("chunk.{n}"))).await?);
            }
            Ok(Bytes::from(out))
        })
    }
